	...
	public void handleBeltEvent(BeltEvent e) {
		if (e.getType() == BeltEvent.EventType.PLATE_CONSUMED) {
			...
			if (plate.getChef() == this) {
				...
				IngredientPortion[] food = plate.getContents().getIngredients();
				
				for(int i = 0; i < food.length; i++)
				{
					eaten += food[i].getAmount();
				}
			...
			{
				balance += plate.getPrice();
				IngredientPortion[] foodSpoil = plate.getContents().getIngredients();
				for(int i = 0; i < foodSpoil.length; i++)
				{
					spoiled += foodSpoil[i].getAmount();
				}
			}
